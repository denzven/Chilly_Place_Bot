...
class Bumprole(Cog):
    ...
    async def disboard_bump(self, message: Message):
        ...
        if "Bump done" not in desc:
            ...
            print(f"{user} bumped {message.guild.name} unsuccessfully")
            embed=discord.Embed(title=f"Thank you for trying to bump the server i am proud of you {user.name}", description=f"you couldn't bump {guild.name} \n \n \n you are given {fake_supporter_role2.mention} for 5mins as a honour", color=0xfe7a7a)
