    ...
    async def on_message(self, message: commands.clean_content):
        ...
        if message.author.id == Minecraft_bot_id:
            if 'Type "/help" for help.' in message.content:
                ...
                embed = discord.Embed(
                    title="test", description="``` testy tests ```", color=0x04FF00
                )
                embed.add_field(
                    name="Server IP:", value="`the_botboi.aternos.me`", inline=False
                )
                embed.set_footer(text="test it bishh")
                await minecraftChat_channel.send(
                    content="<@&841230563843113001> hehe ping pong", embed=embed
                )

            if "Server has started" in message.content:
                embed = discord.Embed(
                    title="✅ The Server has Started ",
                    description="``` the server is now ready to play! ```",
                    color=0x00FF00,
                )
                embed.add_field(
                    name="Server IP:", value="`the_botboi.aternos.me`", inline=False
                ...
                await message.channel.send(
                    content="the Server has Started! <@&841230563843113001>",
                    embed=embed,
                )

            if "Server has stopped" in message.content:
                embed = discord.Embed(
                    title="🟥 The Server has Stoped ",
                    description="``` hope yall had an awesome Time! ```",
                    color=0xFF0000,
                )
                embed.add_field(
                    name="Server IP:", value="`the_botboi.aternos.me`", inline=False
                ...
                await message.channel.send(
                    content="The Server has Stopped <@&841230563843113001>", embed=embed
                )
