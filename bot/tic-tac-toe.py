...
class tictactoe(Cog):
    ...
    async def ttt(self, ctx, member: discord.Member):
        embed = discord.Embed(
            color=0xF5F5F5,
            title=f"Hey, {ctx.author.name} wants to play tic-tac-toe with you!",
        )
        acceptdenycomps = [
            [
        ...
        def haswon(team):
            if (
                selections[0][0] == team
                and selections[0][1] == team
                and selections[0][2] == team
            ):
                return True
            if (
                selections[1][0] == team
                and selections[1][1] == team
                and selections[1][2] == team
            ):
                return True
            if (
                selections[2][0] == team
                and selections[2][1] == team
                and selections[2][2] == team
            ):
                return True
            if (
                selections[0][0] == team
                and selections[1][0] == team
                and selections[2][0] == team
            ):
                return True
            if (
                selections[0][1] == team
        ...
        m = await ctx.send(
            embed=embed, components=acceptdenycomps, content=member.mention
        )

        def confirmcheck(res):
            return (
                res.user.id == member.id
                and res.channel.id == ctx.channel.id
                and str(res.message.id) == str(m.id)
            )

        try:
            res = await self.bot.wait_for(
                "button_click", check=confirmcheck, timeout=50
            )
        ...
        if res.component.label == "Accept":
            ...
            embed = discord.Embed(
                ...
                title=f"{member.name} has accepted!",
            ...
            accept = False
            embed = discord.Embed(
                color=discord.Colour.red(), title=f"{member.name} has declined."
            )
            await m.edit(embed=embed)
            return
        ...
        greensturnembed = discord.Embed(
            color=0xF5F5F5, title=f"{ctx.author.name}'s turn"
        )
        ...
        while accept:
            ...
            if not greenstatus:
                ...
                try:
                    res = await self.bot.wait_for(
                        "button_click", check=redsturncheck, timeout=50
                    )
                    await res.respond(type=6)
                    listid = res.component.id
                    firstpart, secondpart = listid.split(" ")
                    board[int(firstpart)][int(secondpart)] = Button(
                        style=ButtonStyle.red, label="⠀", id="1 0", disabled=True
                    )
