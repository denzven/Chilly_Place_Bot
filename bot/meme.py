...
topasslist = []
with open("bot/memetemplate.json") as file:
    f = json.load(file)
    li = f["data"]["memes"]
    for e in li:
...
class memes(commands.Cog):
    ...
    async def getgeneratedmeme(self, data):
        ...
        r = await self.bot.session.post(url, params=data)
        js = await r.json()
        return js
    ...
    async def create(self, ctx, *, query: str = "none"):
        ...
        if query in ["none", "help"]:
            ...
            st = query.split(":")
            tosearch = st[0].lower()
            with open("bot/memetemplates.json") as file:
                f = json.load(file)
                li = f["data"]["memes"]
                ...
                if not timplist:
                    ...
                    mastdict = {
                        ...
                        "username": self.bot.data['imgflipuser'],
                        "password": self.bot.data['imgflippass'],
                    }
    ...
    async def motiv(self, ctx):
        ...
        try:
            ...
            await self.bot.get_cog("image").to_embed(ctx, img, "Retromeme")
    ...
    async def drake(
        ...
        try:
            ...
            url = f"https://api.alexflipnote.dev/drake?top={ttop}&" \
                  f"bottom={tbot}"
            y = await self.bot.session.get(url)
            z = y.content
            file = await z.read()
            bio = BytesIO(file)
            bio.seek(0)
    ...
    async def modernmeme(self, ctx):
        ...
        puncline/top text/joke for the "
            ...
            try:
                tm = await self.bot.wait_for("message", timeout=60.0,
                                                check=check)
            except asyncio.TimeoutError:
                return await msg.reply(
                    "No top text was provided, If you want me to make your "
                ...
                await self.bot.get_cog("image").to_embed(ctx, img, "Meme")
