...
async def poll(ctx, title, *names):
    ...
    while True:
        ...
        if res.channel == ctx.channel:
            ...
            content = "\n".join(
                map(
                    lambda x: f'`{data["items"][x]["name"]}` : {len(data["items"][x]["users"])} votes',
                    data["items"],
                )
            )
...
async def calc(ctx):
    ...
    while m.created_at < delta:
        try:
            ...
            await m.edit(
                content="Calculator Closed as noone responded",
                embed=e,
                components=[],
                type=7,
            )
...
async def graph3(ctx, formula_og, tags=None):
    ...
    plt.title(
        f"graphical representation of {formula_og} = 0",
        color="w",
        pad=20,
        fontsize="small",
    )
...
async def user_metrics_background_task():
    ...
    while not bot.is_closed():
        try:
            ...
            plt.clf()
            df = pd.read_csv(
                "usermetrics.csv", names=["time", "online", "idle", "offline"]
            )
